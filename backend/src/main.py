...
def create_app(
    ...
    app = FastAPI(
        lifespan=lifespan,
        title="Righteous Rides API",
        debug=settings.DEBUG if settings.DEBUG is not None else settings.APP_STAGE == "dev",
        docs_url=None if settings.APP_STAGE == "prod" else "/api/docs",
        redoc_url=None if settings.APP_STAGE == "prod" else "/api/redoc",
        openapi_url=None if settings.APP_STAGE == "prod" else "/api/openapi.json",
    )
