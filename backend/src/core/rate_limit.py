...
CONTACT_LIMITER_STATE_KEY = "contact_rate_limiter"

AUTH_BUCKET_KEY = "rate-limit:auth"
REFRESH_BUCKET_KEY = "rate-limit:refresh"
CONTACT_BUCKET_KEY = "rate-limit:contact"
...
async def init_rate_limiters(app: FastAPI, redis: Redis) -> None:
    ...
    contact_bucket = await _init_redis_bucket(
        redis,
        bucket_key=CONTACT_BUCKET_KEY,
    ...
    app.state.contact_rate_limiter = Limiter(contact_bucket)


async def public_client_identifier(request: Request) -> str:
